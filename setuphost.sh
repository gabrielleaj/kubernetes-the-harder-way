...
192.168.0.11  control0
192.168.0.12  control1
192.168.0.13  control2
192.168.0.14  worker0
192.168.0.15  worker1
192.168.0.16  worker2
192.168.0.21  kubernetes
...
dhcp-host=52:52:52:00:00:00,192.168.0.10
dhcp-host=52:52:52:00:00:01,192.168.0.11
dhcp-host=52:52:52:00:00:02,192.168.0.12
dhcp-host=52:52:52:00:00:03,192.168.0.13
dhcp-host=52:52:52:00:00:04,192.168.0.14
dhcp-host=52:52:52:00:00:05,192.168.0.15
dhcp-host=52:52:52:00:00:06,192.168.0.16
dhcp-authoritative
domain=kubenet
expand-hosts
EOF
...
case $(uname -s) in
  Darwin)
    user=$(stat -f '%Su' "$dir")
    group=$(stat -f '%Sg' "$dir")
    export="$(realpath "$dir")/nfs-pvs -network 192.168.0.0 -mask 255.255.255.0 -maproot=$user -alldirs"
    ;;
  Linux)
    uid=$(stat -c '%u' "$dir")
    gid=$(stat -c '%g' "$dir")
    user=$(stat -c '%U' "$dir")
    group=$(stat -c '%G' "$dir")
    export="$(realpath "$dir")/nfs-pvs 192.168.0.0/24(rw,root_squash,anonuid=$uid,anongid=$gid,no_subtree_check)"
    ;;
esac
